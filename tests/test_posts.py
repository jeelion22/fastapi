...
def test_get_all_posts(authorized_client, test_posts):
    ...
    def validate(post):
        return schemas.PostWithVote(**post)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("favourit dring", "Tea", False),
        ("favourit food", "Indian meal", True),
    ],
)
...
def test__delete_post_success(authorized_client, test_user, test_posts):
...
def test_unauthrized_user_update_post(client, test_user, test_posts):
...
def test_update_post_non_exist(authorized_client, test_user, test_posts):
    ...
    res = authorized_client.put(f"/posts/{1235}", json=data)
